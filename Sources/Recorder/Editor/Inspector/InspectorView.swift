...
/// selected clip/zoom/layout/mask swaps the tab bar out for `ClipPanel`/`ZoomPanel`/
/// `LayoutPanel`/`MaskPanel`. `‹ Back` (or `Esc`, handled by `TimelineView`'s key handling which
/// clears the same `model.selection`/`selectedClip`) returns.
