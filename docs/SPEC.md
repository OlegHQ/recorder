...
- Preview is direct-manipulation: with a **manual zoom selected**, a rectangle overlay shows the zoom target and can be dragged; with the **Camera tab** open, the camera can be dragged between corners; with a **mask selected**, drag its body to move it or its 8 handles to resize it. Masks/highlights have a one-pixel minimum in the cropped source, independent of preview scale. Small rectangles retain separated 16-point handle hit targets and a body move target; handles connect to the actual geometry. Grabbing anywhere inside a handle preserves the pointer offset. Dragging outside a selected mask does not create a replacement box. Rectangles stay within source bounds; Escape reverts a drag, each completed gesture is one undo step, and arrow keys nudge the selected rectangle (Shift = 10 points).
...
**AC-EXP-1** 1-minute 1080p60 H.264 export finishes in < 30 s on an M-series Mac and plays in QuickTime, Safari and Chrome. **AC-EXP-2** A/V drift at the end of a 10-minute export < 1 frame. **AC-EXP-3** Cancel stops within 1 s and deletes the partial file. **AC-EXP-4** Export duration == `Project.exportDuration` ± 1 frame: the last retained screen or camera media endpoint in output time. Interior gaps remain; trailing gap clips and effects alone do not extend export. Camera intervals count only when the source has camera footage and overlap the playback timeline. Audio mute does not disable video. The sheet duration/size estimate and MP4/GIF encoders use this same boundary; empty/all-gap projects disable Export and Copy to clipboard. Invalid direct exports fail before touching the destination. There is no explicit user in/out export range in v1; playhead and timeline selection do not restrict export.
